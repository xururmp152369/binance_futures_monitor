...
**重要前提**：回測不是獨立實作的策略邏輯，而是直接呼叫 `app/strategy/long_breakout.py` 和 `app/strategy/death_cross_short.py`，並使用 `app/setting/config.py` 的參數。因此，**回測結果完全反映目前的策略設定**，修改 config.py 的任何參數都會同時影響正式系統與回測結果。
...
| 參數 | 類型 | 預設值 | 說明 |
|------|------|--------|------|
| `--strategy` / `-s` | str | 必填 | `long_breakout` \| `death_cross_short` \| `all` |
...
```
1. 初始化符號狀態
   └→ 清空 strategy_state[symbol]、death_cross_state[symbol]
...
```
engine.py
  └→ state_machine.py（on_new_4h_candle / on_new_1h_candle / on_new_daily_candle）
