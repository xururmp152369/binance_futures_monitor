...
_VALID_STRATEGIES = {"long_breakout", "death_cross_short"}
...
def validate_config(data: dict) -> tuple[bool, list[str]]:
    ...
    for field in ("PRD_STRATEGY", "DEV_STRATEGY"):
        ...
        if val is not None:
            if not isinstance(val, list):
                ...
                errors.append(f"`{field}` 包含無效值：{sorted(invalid)}，只接受 long_breakout、death_cross_short")
    ...
    if notify_strategy is not None:
        if not isinstance(notify_strategy, list):
            ...
            errors.append(f"`NOTIFY_STRATEGY` 包含無效值：{sorted(invalid_ns)}，只接受 long_breakout、death_cross_short")
