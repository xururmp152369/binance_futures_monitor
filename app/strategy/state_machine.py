...
def on_new_15m_spike(symbol: str, candle: tuple) -> dict | None:
    """處理新 15m K 棒收盤：檢查 Type 0 量價異動訊號（原突破告警整合版）。

    條件：
    - 量能：當前 K 成交量 > 過去 192 根平均 × VOLUME_THRESHOLD
    - 價格：(close - prev_close) / prev_close × 100 >= PRICE_THRESHOLD（絕對值）
    - 冷卻：距上次 Type 0 告警的 kline close time 間隔 >= ALERT_COOLDOWN

    candle: (open_time_ms, open, high, low, close, quote_volume)
    回傳訊號 dict 或 None。
    """
    rc = models.runtime_config
    ohlc_deque = models.symbol_state.get(symbol, {}).get("kline_15m_ohlc")
    if ohlc_deque is None or len(ohlc_deque) < 193:
        return None

    _open_time_ms, _open, _high, _low, close, volume = candle
    ohlc_list = list(ohlc_deque)
    ...
    if avg_vol <= 0:
        return None
    vol_ratio = volume / avg_vol
    if vol_ratio < rc["VOLUME_THRESHOLD"]:
        return None

    # 價格變化：當前 close vs 前一根 close
    prev_close = ohlc_list[-2][4]
    if prev_close <= 0:
        return None
    price_pct = (close - prev_close) / prev_close * 100
    if abs(price_pct) < rc["PRICE_THRESHOLD"]:
        return None

    # 冷卻檢查（基於 kline close time，與原 send_alert 邏輯一致）
    state = models.symbol_state.get(symbol, {})
    current_kline_time = state.get("last_kline_close_time_15m", 0)
    last_kline_time = models.last_alert.get(symbol, 0)
    if current_kline_time - last_kline_time < rc["ALERT_COOLDOWN"]:
        return None

    # 更新冷卻記錄
    models.last_alert[symbol] = current_kline_time

    # EMA 趨勢（參考用）
    ema_1h = state.get("ema_1h", {})
    ema_4h = state.get("ema_4h", {})

    def _ema_bullish(ema_dict: dict) -> bool | None:
        e15, e30, e45, e60 = ema_dict.get(15), ema_dict.get(30), ema_dict.get(45), ema_dict.get(60)
        if all(v is not None for v in [e15, e30, e45, e60]):
            return e15 > e30 > e45 > e60 and close > e45
        return None

    log.info(
        f"[策略-T0] {symbol} 觸發！close={close:.6f} "
        f"量能={vol_ratio:.1f}× 漲跌={price_pct:+.2f}%"
    )
    return {
        "type":      "type0",
        "symbol":    symbol,
        ...
        "price_pct": price_pct,
        "vol_ratio": vol_ratio,
        "trend_1h":  _ema_bullish(ema_1h),
        "trend_4h":  _ema_bullish(ema_4h),
    }
