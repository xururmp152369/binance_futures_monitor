...
from .exchange import ExchangeConflictError, ExchangeOrderTimeout, create_adapter
...
async def _place_orders_for_user(
    ...
    try:
        log.info(
            f"[自動開單] {symbol} 模式={'正式' if use_prd else '模擬'}"
            f" 交易所={exchange} ({account_name})"
        )

        # 部位上限 & 加倉檢查
        open_positions = await adapter.get_open_positions()
        if is_long:
            side_positions = [p for p in open_positions if p["positionAmt"] > 0]
        ...
        qty_precision, price_precision = await adapter.get_precisions(symbol)
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            ...
            try:
                ...
                fill_price = result["avgPrice"] or entry_price
                _exec = result["executedQty"]
                ...
                break
            except ExchangeOrderTimeout:
                log.warning(
                    f"[自動開單] {symbol} 開倉逾時 attempt={attempt}/{MAX_RETRIES}，查詢訂單..."
                )
                await asyncio.sleep(2)
                try:
                    queried = await adapter.get_order(symbol, order_id)
                    if queried["status"] == "FILLED":
                        fill_price = queried["avgPrice"] or entry_price
                        _exec = queried["executedQty"]
                        filled_qty = _floor_to_precision(
                            _exec if _exec > 0 else qty, qty_precision
                        )
                        log.warning(
                        ...
                        break
                    log.warning(
                        f"[自動開單] {symbol} 訂單未成立 status={queried['status']}，重試開倉..."
                    )
                except Exception:
        ...
        if is_add_on:
            existing_orders = await adapter.get_close_position_orders(symbol, exit_side)
            tp_order = next(
                (o for o in existing_orders if "TAKE_PROFIT" in o.get("type", "")),
                None,
            )
            if tp_order:
                raw_tp = tp_order["stopPrice"]
                if raw_tp > 0:
        ...
        for sl_attempt in range(2):
            try:
                await adapter.create_stop_market_order(
                    symbol,
                    exit_side,
                    round(stop_loss, price_precision),
                    close_position=True,
                )
                log.info(f"[自動開單] {symbol} 止損掛出 stopPrice={stop_loss:.6f} (closePosition)")
                sl_placed = True
                break
            except ExchangeConflictError:
                if sl_attempt == 0:
                    log.warning(f"[自動開單] {symbol} 止損設置條件單衝突，再次清除後重試")
                    await adapter.cancel_close_position_orders(symbol, exit_side)
                    continue
                log.error(f"[自動開單] {symbol} 止損掛出失敗（衝突無法解決）({account_name})")
                warnings.append("⚠️ 止損設置失敗，請手動設置")
                break
                ...
                for tp_attempt in range(2):
                    try:
                        await adapter.create_take_profit_market_order(
                            symbol,
                            exit_side,
                            round(tp_price, price_precision),
                            close_position=True,
                        )
                        log.info(
                        ...
                        if tp_attempt == 0:
                            log.warning(f"[自動開單] {symbol} 止盈{i + 1} 條件單衝突，再次清除後重試")
                            await adapter.cancel_close_position_orders(symbol, exit_side)
                            continue
                        log.error(f"[自動開單] {symbol} 止盈{i + 1} 掛出失敗（衝突無法解決）({account_name})")
                        tp_failed = True
                        break
                    except Exception as e:
